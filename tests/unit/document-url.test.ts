...
describe("sourcePageUrl (actual cited source page)", () => {
  it("points at the document endpoint with the cited page as a query parameter", () => {
    expect(sourcePageUrl("sess-1", "doc-1", "plan-a.pdf", 2)).toBe(
      "/api/document/sess-1/doc-1/plan-a.pdf?page=2",
    );
  });
});
